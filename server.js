...
app.post('/items', async (req, res) => {
  ...
  try {
    ...
    // Get the latest sequential number for this location and month
    const seqResult = await client.query(
      `SELECT serial_number 
       FROM inventory_codes 
       WHERE item_id IN (SELECT id FROM items WHERE location = $1)
         AND kode_inventaris LIKE $2
       ORDER BY kode_inventaris DESC 
       LIMIT 1`,
      [location, `${locationPrefix}${yearSuffix}${month}%`]
    );
    
    // Calculate next sequential number
    let nextSeq = 1;
    if (seqResult.rows.length > 0) {
      const lastSerial = seqResult.rows[0].serial_number;
      nextSeq = parseInt(lastSerial.slice(-3)) + 1;
    }
    
    // Generate complete serial number
    const serialNumber = `${locationPrefix}${yearSuffix}${month}${nextSeq.toString().padStart(3, '0')}`;
    ...
    const itemResult = await client.query(
      'INSERT INTO items (id, name, information, location) VALUES ($1, $2, $3, $4) RETURNING *',
      [id, name, information || '', location]
    );

    // Create inventory code
    await client.query(
      'INSERT INTO inventory_codes (id, item_id, kode_inventaris) VALUES ($1, $2, $3)',
      [uuidv4(), id, serialNumber]
    );
...
app.put('/items/:id', async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(
      'UPDATE items SET name = $1, information = $2, location = $3 WHERE id = $4 RETURNING *',
