...
app.get('/rooms/:id', async (req, res) => {
  try {
    const room = await runQuerySingle('SELECT * FROM rooms WHERE id = $1', [req.params.id]);
    if (!room) {
...
app.post('/rooms', async (req, res) => {
  const { name, description, icon, hidden, replacesDefault } = req.body;
  ...
  try {
    ...
    const id = uuidv4();
    const result = await client.query(
      'INSERT INTO rooms (id, name, description, icon, hidden, replacesDefault) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [id, name, description || '', icon || null, hidden || false, replacesDefault || null]
    );
...
app.put('/rooms/:id', async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(
      'UPDATE rooms SET name = $1, description = $2, icon = $3, hidden = $4, replacesDefault = $5, updated_at = CURRENT_TIMESTAMP WHERE id = $6 RETURNING *',
      [name, description || '', icon || null, hidden || false, replacesDefault || null, req.params.id]
    );
...
app.delete('/rooms/:id', async (req, res) => {
  ...
  try {
    ...
    const result = await client.query('DELETE FROM rooms WHERE id = $1 RETURNING *', [req.params.id]);
